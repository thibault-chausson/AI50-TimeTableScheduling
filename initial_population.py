    ...
    for idx, uv in enumerate(uvs):
        for course_type, time in zip(("cm", "td", "tp"), (uv.cm, uv.td, uv.tp)):
            ...
            while not done:
                ...
                if (planning_r.sum() >= planning_r.shape[0] * planning_r.shape[1] * room_usage) or (counter >= search_cap-1):
                    room_index += 1
                    room = rooms[room_index]
                    planning_r = planning_room(chromosome, room.room)
                ...
                while (planning_r[start_time:start_time+steps, start_day] + planning_t[start_time:start_time+steps, start_day] + planning_u[start_time:start_time+steps, start_day]).sum() != 0:
                    counter += 1
            ...
            gene = Gene(room.room, START_TIME + start_time * MINUTES_PER_CELL, start_day, steps * MINUTES_PER_CELL, teacher, uv.code, course_type)
...
if __name__ == '__main__':
    ...
    uvs = [i for i in uvs if i.teachers != [None]] # Pas de profs pour une UV crée un bug dans la fonction planning_teacher
