...
def sort_dataset(arg_population, arg_capacity_uv_promo_dict, arg_uvs_list, arg_promo, arg_rooms_list):
    ...
    list_fitness = [ft.fitness(chromosome, arg_capacity_uv_promo_dict, arg_uvs_list, arg_promo, arg_rooms_list) for
...
def genetic_algorithm(arg_room_list, arg_population, arg_fitness, room_capacity_dict, capacity_uv_promo_dict, uvs_list,
                      arg_promo, generation=100, mutation_rate=0.01, nb_couple_elite=1, selection_choice='elitiste',
                      crossover_choice='single_point', arg_tournament_size=5, correction=True):
    """
    ...
    for _ in range(generation):
        ...
        for i in range(len(tableau_enfants)):
            arg_fitness, arg_population = rp.replace(arg_fitness, arg_population,
                                                     ft.fitness(tableau_enfants[i], capacity_uv_promo_dict, uvs_list,
